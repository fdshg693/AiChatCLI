...
- `Agents/` は agent 定義、current agent 選択、`AgentToolCatalog`、`OpenAIAgentFactory`、sub-agent 実行、コマンド実行ツールと関連 DTO をまとめます。利用可能 tool は `Tools:Enabled` の base tool 設定と、main agent / sub-agent ごとのコード内ルールを合成して決まります
...
モデルは Function Calling の `command` ツールを使って、ローカルシェルコマンドの実行を要求できます。実際に実行する前に必ずコンソールへコマンドが表示され、ユーザーが `YES` と入力した場合だけ実行されます。

- `command` は既定で main agent / sub-agent の両方へ公開されます
- Windows では PowerShell、その他の OS では `/bin/sh -lc` で実行します
- Windows では PowerShell 実行時に stdout/stderr を UTF-8 にそろえるため、日本語などの Unicode 出力も tool 結果とログで崩れにくくしています
- `timeout_seconds` は既定で `120` 秒、最大 `600` 秒です
- 実行結果は `ok`、`status`、`command`、`exitCode`、`stdout`、`stderr`、`timedOut` などを含む JSON 文字列で返ります
- `NO` の場合、コマンドは実行されず、AI には `status: "denied"` と `denied: true` を含む JSON が返ります
- `NO` の理由を入力した場合だけ、その理由が `reason` として AI に渡されます。理由を空にした場合は拒否された事実だけが返ります
...
ローカルコマンド実行を無効にしたい場合は、`Tools:Enabled` から `command` を外してください。
...
- テキストログの記録内容: セッション開始・終了、ユーザー入力、プロンプト変換結果、モデル送信文、ツール呼び出し（関数名・引数・結果。`command` の拒否結果も含む）、サブエージェント thread ID、AI 応答、スラッシュコマンドとその出力
- thread ログの記録内容: append-only JSONL での thread 作成・接続/切断、ユーザー入力、プロンプト変換結果、モデル送信文、assistant メッセージ、tool 呼び出しと結果、`subagent_invoked` によるサブエージェント thread ID、agent 切り替え時の system prompt snapshot。UTF-8 の日本語などは `\uXXXX` ではなく、そのまま読める形で保存し、tool の入れ子 JSON も `\u0022` ではなく通常の JSON エスケープで残します
