...
- `Bootstrap/`: `AppConfig`、`AppPaths`、`AppComposition` による設定読み込み、runtime path 解決、manual wiring
- `Conversation/`: `ChatLoop`、`ChatTurnPipeline`、`OpenAIChatService`、`ConversationCodec`、`AgentTurnExecutor`
- `Agents/`: agent 定義、tool catalog、OpenAI agent factory、sub-agent 実行、command / read_file / memory / search tool
...
- `Memory/`: durable memory store と Function Calling 用 memory tool
...
2. `AppPaths.Discover(...)` で content root と runtime asset の場所を確定する
...
3. `ChatTurnPipeline` が slash command 判定、template 展開、ログ記録、`IChatService` 呼び出しをまとめる
4. `OpenAIChatService` と `AgentTurnExecutor` がモデル応答と tool 実行を処理する
5. `ThreadSessionManager` と `ChatHistoryLogger` が thread log / text log を記録する

`ChatTurnPipeline` は thread replay 用の structured log と、人間向け transcript の順序をそろえる責務も持ちます。順序を変える変更は replay 破壊につながりやすいため注意してください。
...
- `Tools:Enabled` は base tool の採用可否を決めます
- main agent / sub-agent の公開差は `AgentToolScope` で表現します
- `OpenAIAgentFactory` は main agent と sub-agent の binding を組み立てます
- `SubAgentRunner` は独立コンテキストのサブエージェント実行と sub-agent thread log を担当します

新しい tool を追加するときは、tool 実装を増やすだけでなく、`AgentToolCatalog`、`/status` 表示、README 配下の関連文書、必要な focused test まで同じ変更でそろえてください。
...
- 状態変更系コマンドは `AgentSelection`、`IChatService`、`ThreadSessionManager` の同期を崩さないようにします
