...
- 複数行テキストもそのまま保持されます
- `Logging:ThreadEnabled` が有効なら、起動時に空の current thread が 1 つ自動作成されます
...
テンプレートは既定では `.ai_chat/settings.json` で指定された `Paths:Prompts` の JSON で追加・編集できます。CLI からは `/prompt template ...` で操作します。外部で JSON を直接編集した場合は `/prompt template reload` で再読み込みできます。
...
既定では `.ai_chat/settings.json` で指定された `Paths:Agents` の JSON で追加・編集でき、CLI からは `/agent ...` で操作します。`/agent list` と `/agent show <名前>` では、その agent に許可された tool も確認できます。外部で JSON を直接編集した場合は `/agent reload` で再読み込みできます。
...
`tools` に `skill` を含めると、`.ai_chat/settings.json` で指定された `Paths:SkillsDirectory` 配下の `SKILL.md` にある skill の `name` / `description` だけが system prompt に追加されます。本文は必要になったときだけ `skill` ツール呼び出しで取得されます。
...
会話は thread 単位で構造化ログに保存され、`Logging:ThreadEnabled` が有効なときは起動時に current thread が自動作成されます。あとから `/thread use <id>` で読み込んで継続できます。
...
- `Logging:ThreadEnabled=false` にすると thread 機能と main thread JSONL は無効になります
- `Logging:SubAgentThreadEnabled=false` にすると sub-agent JSONL だけを止められます
- `/agent use`、`/agent reload`、`/thread use`、`/thread new` のような状態変更は thread ログにも記録されます

## 次に読むべき文書

- agent ごとの tool 定義、`.ai_chat/settings.json`、`appsettings.local.json`、`Paths:*` は [`configuration.md`](configuration.md)
